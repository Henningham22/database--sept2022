...
SELECT * FROM orders;
ALTER TABLE orders
ADD FOREIGN KEY (customer_id) REFERENCES customers (cust_id) ON DELETE CASCADE;
...
select * from items WHERE item_name LIKE 'q%';
select * from items WHERE item_price =8;

SELECT oi_id FROM orders_items WHERE oi_id=3;
SELECT oi_id FROM orders_items WHERE oi_id= (SELECT orders_id FROM orders WHERE orders_id=2);

SELECT * FROM customers JOIN orders on customers.cust_id= orders.customer_id;